...
from core.warband.wb_constants import WARBAND_PATH
from ui import logo
from ui.addons_menu import AddonsMenu
from ui.main_menu import PatchMenu
from ui.ui_utils import (INDENTATION_AMOUNT, clear_screen, colors,
                         format_indented, print_indented)
...
if WARBAND_PATH is None:
    ...
    raw_input(format_indented(
        "Press Enter to exit...", INDENTATION_AMOUNT+1, True))
    sys.exit(1)
...
try:
    ...
    print_indented(colors.format_error(
        "Invalid URL: {0}".format(API)), new_line_before=True)
    raw_input(format_indented("Press Enter to exit...",
                              INDENTATION_AMOUNT + 1, True))
    sys.exit(1)
except urllib2.HTTPError as e:
    print_indented(colors.format_error(
        "Server failed to fulfill the request."), new_line_before=True)
    ...
    print_indented(colors.format_error(
        "Invalid server response code " + e.line), new_line_before=True)
    raw_input(format_indented("Press Enter to exit...",
                              INDENTATION_AMOUNT + 1, True))
    sys.exit(1)
except urllib2.URLError as e:
    print_indented(colors.format_error(
        "Cannot reach server. Try again later."), new_line_before=True)
